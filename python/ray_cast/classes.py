...
class Boundary:
    ...
    def draw(self, win: pg.Surface, stroke: float = 1):
...
class Ray:
    ...
    def cast(self, wall: Boundary):
        ...
        x3 = self.pos.x
        y3 = self.pos.y
        x4 = self.pos.x + self.ang.x
        y4 = self.pos.y + self.ang.y
...
class Particle:
    ...
    def look(self, win: pg.Surface, walls: List[Boundary], do_draw: bool):
        scene = []
        for ray in self.rays:
            closest = None
            record = [float("inf")]
    ...
    def draw(self, win, color=(255, 255, 255), stroke: float = 0):
